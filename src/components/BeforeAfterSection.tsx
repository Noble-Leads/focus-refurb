...
const BeforeAfterSection = ({
  ...
  const multiColumn = pairs.length > 1;

  return (
    <section className="py-10 md:py-14 bg-background">
      <div className={`container ${multiColumn ? "max-w-6xl" : "max-w-4xl"}`}>
        <ScrollReveal>
          {eyebrow && (
        ...
        <div
          className={
            multiColumn
              ? "grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-6 lg:gap-8"
              : "space-y-10 md:space-y-14"
          }
        >
          {pairs.map((pair) => (
            <div key={pair.label ?? pair.before.src} className="min-w-0">
              {pair.label && (
